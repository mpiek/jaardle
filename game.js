...
// Bord van 7 getrapte reeksen (brons→diamant) + 9 trofeeën + het
...
// sectie) en 3 strikt eenmalig (flawless, rematch, saver — bewust: een teller
...
function ensureAchvLocal() {
  ...
  // rematch is niet uit de historie te reconstrueren (vrij spel werd anoniem nooit
  // bewaard) → begint leeg.
  let run = 0, best = 0;
...
function recordAchvLocal() {
  ...
  if (state.won) {
    ...
    (l.lostDaily = l.lostDaily || {})[y] = true;   // verloren daily-jaar onthouden voor de revanche
  }
  saveAchvLocal(l);
}
