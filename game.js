...
function renderAchvBoard(body, a) {
  ...
  const repeatable = shown.filter((tr) => tr.repeatable).map((tr) => achvTrophyHtml(a, tr)).join("");
  const milestones = shown.filter((tr) => !tr.repeatable).map((tr) => achvTrophyHtml(a, tr)).join("");
  body.innerHTML = `
    ${capstoneBarHtml(a)}
